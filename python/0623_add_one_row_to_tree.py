...
class Solution:
    def addOneRow(self, root: TreeNode, v: int, d: int) -> TreeNode:
        def _traversal(prenode, treenode, is_left=True, depth=1):
            if treenode or depth == d:
                cur = treenode
                if depth == d:
                    newnode = TreeNode(v)
                    if is_left:
                    ...
                    cur = newnode
                _traversal(treenode, cur.left, True, depth + 1)
                _traversal(treenode, cur.right, False, depth + 1)

        if d > 1:
            _traversal(root, root)
        else:
            newnode = TreeNode(v)
            newnode.left = root
            root = newnode
